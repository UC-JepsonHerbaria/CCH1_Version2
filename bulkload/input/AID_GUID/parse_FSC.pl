...
#check for nulls
	...
	$otherCatalogNumbers =~ s/^(\d+); *(\d+)$/$1$2/ig; #there series of bad records that have semi-colons where a comma is on label
	$otherCatalogNumbers =~ s/(\d+) [a-z]$/$1/g;
...
if (length ($otherCatalogNumbers) >= 1){ #FSC was not in CCH, so there is no old resources to map to their records
	...
	elsif ($otherCatalogNumbers =~ m/^([0-9]+-[0-9]+)$/){ #unique to FSC
		$old_AID="";
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^FSC(0*[1-9][0-9]*)$/){
