...
my $oldA="";
	$catalogNumber =~ s/^fsc/FSC/ig; #there is one bad record here that needed fixed
	$otherCatalogNumbers =~ s/^335; 14,710/14710/ig; #there is one bad record here that needed fixed


#extract old herbarium and aid numbers
if (length ($otherCatalogNumbers) >= 1){ #FSC was not in CCH, so there is no old resources to map to their records

	if ($otherCatalogNumbers =~ m/^([0-9]+)[.,]([0-9]+)$/){ #unique to FSC, it has commas in some of the other field, and no herbarium codes
		$old_AID=$herb.$1.$2;
		$old_AID =~ s/ //g;
	...
	elsif ($otherCatalogNumbers =~ m/^([0-9]+-[0-9]+)$/){ #unique to FSC, it has commas in some of the other field, and no herbarium codes
		$old_AID="";
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(FSC)(\d+)$/){#ignore leading zeros, do not delete, there is no mapping to CCH records for FSC
...
#find duplicates
if ($old_AID !~ m/^ *$/){
	if($duplicate{$old_AID}++){#check barcodes for duplciates, since no CCH resources are present
	++$dups;
