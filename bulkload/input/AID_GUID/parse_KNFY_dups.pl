...
#ELH appears not to have anything in othercatalogNumbers
...
#construct catalog numbers
	if ($catalogNumber =~ m/^KNFY(0*[1-9][0-9]*)[A-Za-z-]+$/){
	...
	elsif ($catalogNumber =~ m/^KNFY(0*[1-9][0-9]*)$/){
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
		$oldA = $old_AID = "";
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		...
		$oldA = $old_AID = "";
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		#print "HERB(5)\t$ALT_CCH_BARCODE\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
	}
	elsif ($catalogNumber =~ m/^(0*[1-9][0-9]*)$/){#fix records without an herbarium code
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
		$oldA = $old_AID = "";
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		#print "HERB(6)\t$ALT_CCH_BARCODE\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
	}
	elsif ($catalogNumber =~ m/^(NULL| *)$/){
		$catalogNumber = "NULL";
		$old_AID = $herb."-".$CCH2id."-TEMPAID"; 
		#this is a dummy replacement until an actual barcode is added
		$oldA = $ALT_CCH_BARCODE = "";
