...
#skip otherCatNumbers records with catalog numbers from the SCIR Herbarium as the other cat numbers are a different series than the UCSB ones but still are coming up duplicates

	if ($otherCatalogNumbers =~ m/NULL/){
		$otherCatalogNumbers = "";
		$old_AID="";
	}
	elsif ($otherCatalogNumbers =~ m/^(\d+)[AB]?$/){ 
	...
	elsif ($otherCatalogNumbers =~ m/^(UCSB)(\d+)$/){ 
		$old_AID = $1.$2; #add the herbarium code to fix these errors and have a version with leading zeros
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	else{
...
#the alt catalog numbers do not need leading zeros removed
if (length ($catalogNumber) >= 1){

	if ($catalogNumber =~ m/^(UCSB)(\d+)$/){
		...
		$ALT_CCH_BARCODE = $1.$2;
		$old_AID = "";  #add the herbarium code to fix these errors and have a version with leading zeros
	}
