...
#check for nulls
	...
	$catalogNumber =~ s/^(SBBG171999SBBG172219|SBBG167B58|SBBG1W)$/NULL/;#delete this single repetitive accession that is showing across multiple records; looks like an error
	$catalogNumber =~ s/^\+?(sbbg|SBB|BBG)(\d+)/SBBG$2/;#
	$catalogNumber =~ s/['`]//g;#
	$catalogNumber =~ s/^141 +393/141393/;#
$otherCatalogNumbers =~ s/^(CHIS[\:\- ]*|q)$//;#delete this single repetitive accession that is showing across multiple records; looks like an error
$otherCatalogNumbers =~ s/; (CHIS-?|937\.)$//;
$otherCatalogNumbers =~ s/^['`]$//;#
$otherCatalogNumbers =~ s/['`]//g;#
$otherCatalogNumbers =~ s/^(2 Apr 1992|122273SBBG170709|44363SBBG162920)$//;#problem number with blank CatNumber record
$otherCatalogNumbers =~ s/^29550 CHIS-00375: 16616$/29550/;
$otherCatalogNumbers =~ s/^18030;CHIS-00375: 14438$/18030/;
$otherCatalogNumbers =~ s/^CABR-00385; 2651$/2651/;
$otherCatalogNumbers =~ s/^101582; CHIS00375: 14395$/101582/;


#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^SBBG0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
		$oldA =~ s/[A-Za-z]+$//;
	}
	elsif ($otherCatalogNumbers =~ m/^SBBG([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^([1-9][0-9]*); +(CHIS-\d+): *(\d+)[:;] *JSTOR.+$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^CHIS-\d+: *\d+; (\d+)$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^([1-9][0-9]*); +(CHIS|CABR)-\d+[:;] *(\d+)$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^([1-9][0-9]*); +JSTOR.+$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^(CABR|CHIS)-\d+: *(0*)([1-9][0-9]*)$/){ 
		$old_AID = "";
		$oldA  = "";
	}
	elsif ($otherCatalogNumbers =~ m/^([1-9][0-9]*)$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^SBBG(0*[1-9][0-9]*)$/){
