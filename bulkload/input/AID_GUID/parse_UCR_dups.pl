...
#check for nulls
	...
	$otherCatalogNumbers =~ s/^UCR-$//i;
	$catalogNumber =~ s/^(ucr|r|cr)(\d)/UCR$2/i; #fix some lack of capitalizations in UCR accessions
	next if ($catalogNumber =~ m/^(UCR0110798 .+|UCR0084267 .+|UCR0078070 291187|UCR0065896 44197|UCR0083UCR0084836 901|UCR0082810 f|UCR0082912 UCR0082911|UCR0096653 UCR0096653)$/);#delete this problem accession


#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^UCR-?0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
	...
	elsif ($otherCatalogNumbers =~ m/^UCR-?([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$oldA = "";
		$duplicate_OTH{$old_AID}++;
		#print "HERB(2a)\t$oldA\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n"";
	...
	elsif ($otherCatalogNumbers =~ m/^UCR-?([0-9]+)\.[1-9]*$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
		$duplicate_OTH{$old_AID}++;
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^UCR(0*[1-9][0-9]*)[A-Za-z]*$/){
