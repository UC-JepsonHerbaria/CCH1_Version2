...
my $oldA="";
	...
	$catalogNumber =~ s/^(ucr|r|cr)/UCR/i; #fix some lack of capitalizations in UCR accessions
	$catalogNumber =~ s/^(UCR0110798 .+|UCR0084267 .+|UCR0078070 291187|UCR0065896 44197|UCR0083UCR0084836 901|UCR0082810 f|UCR0082912 UCR0082911|UCR0096653 UCR0096653)$/NULL/;#delete this problem accession
...
if (length ($otherCatalogNumbers) >= 1){
	...
	elsif ($otherCatalogNumbers =~ m/^(UCR)-(\d+)[AXxBCDEFGHIJ]?$/){ 
		$old_AID=$1.$2;
