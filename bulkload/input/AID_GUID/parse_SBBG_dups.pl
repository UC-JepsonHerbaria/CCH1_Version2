...
$oldA="";
	...
	$catalogNumber =~ s/^(sbbg|SBB|BBG)\d/SBBG/;#
	$catalogNumber =~ s/['`]//;#
	$catalogNumber =~ s/^141 +393/141393/;#
$otherCatalogNumbers =~ s/^(CHIS[\:\- ]*|q)$/NULL/;#delete this single repetitive accession that is showing across multiple records; looks like an error
$otherCatalogNumbers =~ s/; (CHIS-?|937\.)$//;
$otherCatalogNumbers =~ s/^['`]$/NULL/;#
$otherCatalogNumbers =~ s/['`]//g;#
$otherCatalogNumbers =~ s/^(2 Apr 1992|122273SBBG170709|44363SBBG162920)$/NULL/;#problem number with blank CatNumber record
...
if (length ($otherCatalogNumbers) >= 1){
	...
	elsif ($otherCatalogNumbers =~ m/^(\d+)$/){ 
		$old_AID=$herb.$1;
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($otherCatalogNumbers =~ m/^SBBG(\d+)$/){ 
		$old_AID=$herb.$1;
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	else{
