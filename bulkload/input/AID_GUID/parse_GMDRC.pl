...
#fix some data quality and formatting problems that make import of fields of certain records problematic
#if (m/\"(N\d\d)[^ ]+ /){
s/\"(N\d\d)[^ ]+ (\d\d\.\d+)\t\' (W\d\d\d)[^ ]+ (\d\d\.\d+)\t\'(; \d\d)[^ ]+ (\d\d\.\d+m) (-?\d\d\d)[^ ]+ (\d\d\.\d+m)\"/$1 $2 $3 $4$5d $6 $7d $8/g;
#print $1.$2.$3.$4.$5.$6.$7.$8."\n";
#"N35º 15.340	' W114º 58.418	'; 35§ 15.340m 114§ 58.418m"
#}	
...
if (length ($otherCatalogNumbers) >= 1){ #GMDRC does not have other catalog numbers filled out currently, old_aid=CatalogNumber
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^GMDRC(\d+)[AaBb]?$/){
		$ALT_CCH_BARCODE="";
		$old_AID = $herb.$1;
