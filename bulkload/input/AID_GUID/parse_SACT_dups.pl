...
#SACT had records in CCH1, mostly without leading zeros
#SACT was last last loaded in 2013 by Dick.  I used the same file as he created and never updated the loading script
#Leading zeros were left in Dick's original parsed file, but it is possible that leading zeros were removed occasionally at other stages of loading 
if (length ($otherCatalogNumbers) >= 1){
	...
	if ($otherCatalogNumbers =~ m/NULL/){
		...
		$old_AID = $herb.$1; #add the herbarium code to fix these errors and have a version with leading zeros
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($otherCatalogNumbers =~ m/^SACT(\d+)$/){ 
		$old_AID = $herb.$1; #have a version with leading zeros
		#print "HERB(2)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	else{
...
#there is currently nothing in other cat numbers for SACT for most records


	
#construct catalog numbers
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^(SACT)(\d+)$/){
		$old_AID = $1.$2;
		$ALT_CCH_BARCODE = "";
