...
#LVNP appears not to have anything in othercatalogNumbers
	if ($otherCatalogNumbers =~ m/^(LAVO) *(\d+)$/){
		$old_AID = $1.$2;
		$oldA = "";
		if ($catalogNumber =~ m/^(NULL| *)$/){#some of these do not have a barcode
			$ALT_CCH_BARCODE = $herb."-".$old_AID."-BARCODE";
			$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		}
	}
	elsif ($otherCatalogNumbers =~ m/^(LAVO)-(\d+)$/){
		$old_AID = $1.$2;
		$oldA = "";
		if ($catalogNumber =~ m/^(NULL| *)$/){#some of these do not have a barcode
			$ALT_CCH_BARCODE = $herb."-".$old_AID."-BARCODE";
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(LAVO) *([0-9][0-9]*)[A-Za-z-]+$/){
		$old_AID = $1.$2;
	...
	elsif ($catalogNumber =~ m/^(LAVO) *([0-9][0-9]*)$/){
		$old_AID = $1.$2;
		$ALT_CCH_BARCODE = $herb."-".$1.$2."-BARCODE";
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		#print "HERB(3)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
