...
#check for nulls
	...
	$otherCatalogNumbers =~ s/^(y\.?\d+|495-54|195-54)$//;#delete this single repetitive accession that is showing across multiple records; looks like an error
	$otherCatalogNumbers =~ s/^P\d+$//;#delete this repetitive accession that is showing across multiple records; looks like an error
	$otherCatalogNumbers =~ s/^(Peirson Herbarium No\.10576|frag pkt 3011|5170 of.+|1845 of.+)$//; #delete problem accessions

#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){
#PASA has nothing in othercatalogNumbers
	if ($otherCatalogNumbers =~ m/^[A-Za-z0-9-]+$/){
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^PASA_?(0+)([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$2;
		$oldA = $catalogNumber;
		$ALT_CCH_BARCODE = "";
		$duplicate_CAT{$old_AID}++;#count to find duplicates
		#print "HERB(3)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^PASA_?([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$1;
		$oldA = "";
		$ALT_CCH_BARCODE = "";
		$duplicate_CAT{$old_AID}++;#count to find duplicates
	...
	elsif ($catalogNumber =~ m/^H-?(0+)([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$2;
		$oldA = $catalogNumber;
		$ALT_CCH_BARCODE = "";
		$duplicate_CAT{$old_AID}++;#count to find duplicates
	...
	elsif ($catalogNumber =~ m/^H-?([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$1;
		$oldA = $catalogNumber;
		$ALT_CCH_BARCODE = "";
		$duplicate_CAT{$old_AID}++;#count to find duplicates
...
#exclude ALL duplicates 
	if ($duplicate_FOUND_CAT{$old_AID}) {
