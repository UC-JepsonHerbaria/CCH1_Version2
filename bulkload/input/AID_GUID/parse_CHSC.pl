...
my $oldA="";
	$catalogNumber =~ s/^chsc/CHSC/i; #fix some lack of capitalizations in CHSC accessions
	$catalogNumber =~ s/^CHSC0629S6$/NULL/;#delete this problem accession

#extract old herbarium and aid numbers
#most if not all CHSC accession have no other catalog numbers, the are assigning barcodes as original accessions
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(CHSC) (\d+)$/){
		#CHSC has leading zeros in CCH2, while it more than likely never had them in CCH1
		#need to make an alternative variant wihtout leading 0's to make the CCH1 accession ID's
		#some loads erroneously left in leading zeros, like what happened with UCSC, so some CCH1 data assets may include zeros
		$ALT_CCH_BARCODE=$1."-BC".$2;
		$old_AID=$1.$2;
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(CHSC)(\d+)$/){
		#CHSC has leading zeros in CCH2, while it more than likely never had them in CCH1
		#need to make an alternative variant wihtout leading 0's to make the CCH1 accession ID's
		#some loads erroneously left in leading zeros, like what happened with UCSC, so some CCH1 data assets may include zeros
		$ALT_CCH_BARCODE=$1."-BC".$2;
