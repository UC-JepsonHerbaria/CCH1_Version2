...
#extract old herbarium and aid numbers
   if (($otherCatalogNumbers =~ m/^BLMAR\d+/) && ($catalogNumber =~ m/^ARF\d+/)){
	...
	if ($catalogNumber =~ m/^(ARF)([0-9][0-9]*)[A-Za-z]*$/){ 
		...
		#print "HERB(2)\t$ALT_CCH_BARCODE\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	else{
		&CCH::log_change("BAD catalogNumber==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\t$_");
		print "BAD catalogNumber(1)==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		print DUPLOG "BAD catalogNumber(1)==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		$catalogNumber = "";
		$ALT_CCH_BARCODE="";
	}
   }
	...
	if ($catalogNumber =~ m/^BLMAR([1-9][0-9]+)$/){ 
		...
		print "BAD catalogNumber(3)==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		$otherCatalogNumbers = "";
   ...
   if (length ($catalogNumber) >= 1){#old_AID is the old CCH1 accession number
...
my ($dups,$dups_B,$ALTER, $ORTH, %DUP_FOUND, $count_record) = "";
