...
#KNFY appears not to have anything in othercatalogNumbers, they also never had records in CCH1
#KNFY only has a single barcode and no catalog numbers, so a dummy CCH1 catalog number is created
#in the file KNFY_AID_GUID.txt, which will be used internally but not be displayed in CCH1.
#as barcodes are entered, these will replace this dummy number
...
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^KNFY(\d+)$/){
		$aid=$1;
		$aidcode=$herb;
		$ALT_CCH_BARCODE=$ADD{$occurrenceID};
...
#keep old_aid blank since it was not in CCH1
if ($old_AID =~ m/^ *$/){
...
#activate this when barcodes are entered as Catalognumbers in KNFY
