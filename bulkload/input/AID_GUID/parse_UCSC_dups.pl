...
#check for nulls
	...
	#databse changed, this format not used exxcept for two anomalous times

	#$catalogNumber =~ s/^(UCSC100002219, .+|UCSC100002217, .+)$/NULL/;#NULL these problem accessions

	$otherCatalogNumbers =~ s/Calilfornia/California/;
	$otherCatalogNumbers =~ s/, Lemmon Herbarium.+$//;
	$otherCatalogNumbers =~ s/^Herbarium of the University of California: 1902; *//;
	$otherCatalogNumbers =~ s/^308, 14727$/UC14727; UCSC308/;
	$otherCatalogNumbers =~ s/^.+ink-stamped UCSC accession number: (\d+);.+$/UCSC$1/;
	$otherCatalogNumbers =~ s/.+ink-stamped UCSC accession number: (\d+)$/UCSC$1/;
	$otherCatalogNumbers =~ s/^ink-stamped UCSC accession number: (\d+);.+$/UCSC$1/;

	$otherCatalogNumbers =~ s/^Herbarium of the University of California: (\d+); (\d+)$/UC$1; UCSC$2/i;
	$otherCatalogNumbers =~ s/^(\d+); *UC.+\d+; [A-Z]+\d/UCSC$1/;
	$otherCatalogNumbers =~ s/^(\d+); *Herbarium of the University of California: (\d+)$/UC$2; UCSC$1/i;
	$otherCatalogNumbers =~ s/^UC\d+; *(UCSC\d+);.+/$1/;
...
#construct the 3rd variant, which is a form without leading zeros
	...
	#add leading zeros for the alternate old CCH AID, CCH2 removed those zeros when loaded
		if ($otherID =~ m/^(\d\d\d\d\d[A-Za-z]*)$/){
			$oldA = $herb."0".$otherID;
		#print "HERB(1a)\t$oldA==>$otherCatalogNumbers==>$oldA\n";
		}
		elsif ($otherID =~ m/^(\d\d\d\d[A-Za-z]*)$/){
			$oldA = $herb."00".$otherID;
		#print "HERB(1b)\t$oldA==>$otherCatalogNumbers==>$oldA\n";
		}
		elsif ($otherID =~ m/^(\d\d\d[A-Za-z]*)$/){
			$oldA = $herb."000".$otherID;
		#print "HERB(1c)\t$oldA==>$otherCatalogNumbers==>$oldA\n";
		}
		elsif ($otherID =~ m/^(\d\d[A-Za-z]*)$/){
			$oldA = $herb."0000".$otherID;
		#print "HERB(1d)\t$oldA==>$otherCatalogNumbers==>$oldA\n";
		}
		elsif ($otherID =~ m/^(\d[A-Za-z]*)$/){
			$oldA = $herb."00000".$otherID;
		#print "HERB(1e)\t$oldA==>$otherCatalogNumbers==>$oldA\n";
		}
		else{
			$oldA = "";
		}
	...
	elsif ($otherCatalogNumbers =~ m/^UC\d+; UCSC(\d+)$/){ 
		$old_AID = $herb.$1;
		$otherID = $1;
		$duplicate_OTH{$old_AID}++;
		#print "HERB(2a)\t$oldA\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n"";
