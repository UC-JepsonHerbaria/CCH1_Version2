...
$catalogNumber =~ s/^HNT$//i;

#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){ #HNT was not in CCH, so there is no old resources to map to their records
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^HNT(0*[1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$ALT_CCH_BARCODE = "";
	...
	elsif ($catalogNumber =~ m/^(0*[1-9][0-9]*)[A-Za-z]*$/){#fix records without an herbarium code, HNT has additional letters at the end
		$old_AID = $herb.$1;
		$duplicate_CAT{$old_AID}++;#count to find duplicates
		#print "HERB(3)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
	}
