...
#HNT also never had records in CCH1
if (length ($otherCatalogNumbers) >= 1){
	if ($otherCatalogNumbers =~ m/^(\d+)[AB]?$/i){
		$oldA=$herb.$1;
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(HNT)(\d+)[AB]?$/i){
		$old_AID=$1.$2;
		$ALT_CCH_BARCODE="";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(\d+)[ABCDd]?r?$/i){
		$old_AID=$herb.$1;
		$ALT_CCH_BARCODE="";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
