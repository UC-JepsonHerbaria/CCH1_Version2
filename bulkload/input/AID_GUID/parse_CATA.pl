...
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^(CATA)(\d+)$/){
		$ALT_CCH_BARCODE="";
		$old_AID=$catalogNumber;
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(\d+)$/){ #the last snapshot from CATA did not have herbairum code with the catalogNumber
		$ALT_CCH_BARCODE="";
		$old_AID=$herb.$1;
		#print "HERB(4)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
