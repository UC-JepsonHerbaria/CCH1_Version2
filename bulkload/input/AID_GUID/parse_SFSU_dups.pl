...
#do not need to remove leading zeros as SFSU has not been in CCH1 previously
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	#delete extraeous characters
	$catalogNumber =~ s/`+//;
	$catalogNumber =~ s/^(SFS\.\#VP|ww32SFSU-VP|eSFSU-VP|S2SU-VP)/SFSU-VP/;

	if ($catalogNumber =~ m/^SFSU-VP-(\d+)$/){
