...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
		...
		if ($catalogNumber =~ m/^(pua|PuA|\/UA)(\d+)$/){
	...
	if ($catalogNumber =~ m/^PUA(0*[1-9][0-9]*)$/){
