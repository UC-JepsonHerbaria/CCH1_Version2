...
#YOSE is not yet assigning barcodes or live in Symbiota
#all numbers should be in catalog number, the other catalog number is from other institutions, other number types, and other notes
if (length ($otherCatalogNumbers) >= 1){
	...
	elsif ($otherCatalogNumbers =~ m/^(JU|TR|P[IOSH]|A[SL]|C[AU]|S[AUTC]|E[PR][HLV]|MIN||HERB|Herb|SHEET|Bry|Old|Study|Knapp|Object|[#0O -]*\d+|JEPS\d|UC\d|UC[xX]|PLOT)/){
		$old_AID="";
	}
	else{
