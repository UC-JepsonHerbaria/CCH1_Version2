...
#check for nulls
	...
	$otherCatalogNumbers =~ s/RSA accession \# /RSA accession #: /i;
	$otherCatalogNumbers =~ s/RSA accession: /RSA accession #: /i;
	$otherCatalogNumbers =~ s/RSA Accession\#: /RSA accession #: /;
	
#extract old herbarium and aid numbers
if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^(POM) *accession *\#: *(0+)([1-9][0-9]*)([A-Za-z]*);?.*$/){
		$old_AID = $1.$3;
		$oldA = $1.$2.$3.$4;
	}
	elsif ($otherCatalogNumbers =~ m/^(POM) *accession *\#: *([1-9][0-9]*)([A-Za-z]+);?.*$/){
		$old_AID = $1.$2;
		$oldA = $1.$2.$3.$4;
	}
	elsif ($otherCatalogNumbers =~ m/^(POM) *accession *\#: *([1-9][0-9]*);?.*$/){
		$old_AID = $1.$2;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^(RSA) *accession *\#: *(0+)([1-9][0-9]*)([A-Za-z]*);?.*$/){
		$old_AID = $1.$3;
		$oldA = $1.$2.$3.$4;
	}
	elsif ($otherCatalogNumbers =~ m/^(RSA) *accession *\#: *([1-9][0-9]*)([A-Za-z]+);?.*$/){
		$old_AID = $1.$2;
		$oldA = $1.$2.$3.$4;
	}
	elsif ($otherCatalogNumbers =~ m/^(RSA) *accession *\#: *([1-9][0-9]*);?.*$/){
		$old_AID = $1.$2;
		$oldA = "";
	}
