...
my $oldA="";
	$catalogNumber =~ s/^csusb/CSUSB/i; #fix some lack of capitalizations in CSUSB accessions
	$otherCatalogNumbers =~ s/^csusb/CSUSB/i; #fix some lack of capitalizations in CSUSB accessions


#extract old herbarium and aid numbers
if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^(CSUSB)-([0-9]+)$/){ #CSUSB has dashes, like several others: CSUSB-54
	...
	elsif ($otherCatalogNumbers =~ m/^(CSUSB)-([0-9]+)-?[A-Z]$/){ #CSUSB has dashes, like several others: CSUSB-4177-D
														#remove dashes for CCH1 accessions
		$old_AID=$1.$2;
		#print "HERB(1)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/DS 98/g;
	}
	elsif ($otherCatalogNumbers =~ m/^(CSUSB)([0-9]+)$/){ #CSUSB has a few without dashes: CSUSB4333
		$old_AID=$1.$2;
		#print "HERB(1)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/DS 98/g;
	}
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(CSUSB)(\d+)$/){
		$ALT_CCH_BARCODE = $1."-BC".$2;
		#print "HERB(1)\t$ALT_CCH_BARCODE\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $catalogNumber =~ m/98/g;
	}
