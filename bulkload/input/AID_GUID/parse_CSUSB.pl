...
#construct the 3rd variant, which is a form without leading zeros
	if ($otherCatalogNumbers =~ m/^CSUSB-(0+)([1-9][0-9]*)-?[A-Za-z]*$/){
		$old_AID = $herb.$2;
		$oldA = $otherCatalogNumbers;
	}
	elsif ($otherCatalogNumbers =~ m/^CSUSB-([1-9][0-9]*)-?[A-Za-z]*$/){ 
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
	}
	elsif ($otherCatalogNumbers =~ m/^[CSUSB]*(0*)([1-9][0-9]*)-?[A-Za-z]*$/){ 
		$old_AID = $herb.$2;
		$oldA = $herb.$1.$2;
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^CSUSB(0*[1-9][0-9]*)$/){
