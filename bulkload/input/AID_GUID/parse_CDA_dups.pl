...
#check for nulls
	...
	$catalogNumber =~ s/^cda/CDA/i;

#extract old herbarium and aid numbers
if (length ($otherCatalogNumbers) >= 1){
	if ($otherCatalogNumbers =~ m/^(CDA)-(0+)([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $1.$3;
		$oldA = $1.$2.$3;
		$duplicate_OTH{$old_AID}++;
	}
	elsif ($otherCatalogNumbers =~ m/^(CDA)-([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $1.$2;
		$oldA = "";
		$duplicate_OTH{$old_AID}++;
	}
	elsif ($otherCatalogNumbers =~ m/^(CDA)-(0+)([1-9][0-9]*)[A-Za-z]*[;,].*$/){
		$old_AID = $1.$3;
		$oldA = $1.$2.$3;
		$duplicate_OTH{$old_AID}++;
	}
	elsif ($otherCatalogNumbers =~ m/^(CDA-S|UC|JEPS) *([0-9]+)[A-Za-z]*[;,]*.*$/){
#CDA-S 59381
		$oldA = $old_AID = "";
...
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^CDA-(0+)([1-9][0-9]*)$/){#
	...
	elsif ($catalogNumber =~ m/^CDA-([1-9][0-9]*)$/){#
...
#construct catalog numbers
#the new upload of CDA has the otherCatNum field mostly empty.  
#use the catlognumber to construct the old_AID, otherwise nothing will link to 2018 CDA data
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^CDA-(0+)([1-9][0-9]*)$/){#
		$ALT_CCH_BARCODE = $herb.$1.$2."-BARCODE";
		$old_AID = $herb.$2;
