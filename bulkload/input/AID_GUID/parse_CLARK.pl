...
if (length ($otherCatalogNumbers) >= 1){
	if ($otherCatalogNumbers =~ m/^(A[0-9-]+)$/){ #unique to CLARK, example: A1045-753
		$herbcode="CLARK-";
		$oldA=$herbcode.$1;
...
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^(CLARK-)(A[0-9-]+)[AB]?$/){#unique to CLARK, it has multiple dashes in some accessions
		$old_AID=$1.$2;
		$ALT_CCH_BARCODE="";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
