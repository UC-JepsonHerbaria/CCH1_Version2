...
#JROH does not have anything in otherCatalogNumbers
...
if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^(JROH)(0+)(\d+)[ABC]?$/i){
		$old_AID = $1.$3;
		$oldA = $1.$2.$3;
		$ALT_CCH_BARCODE="NULL";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(JROH)(\d+)[ABC]?$/i){
		$old_AID = $1.$2;
		$ALT_CCH_BARCODE="NULL";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(0+)(\d+)[ABC]?$/i){
		$old_AID = $herb.$2;
		$oldA = $herb.$1.$2;
		$ALT_CCH_BARCODE="NULL";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(\d+)[ABC]?$/i){
		$old_AID = $herb.$1;
		$ALT_CCH_BARCODE="NULL";
		#print "HERB(3)\t$herbcode$aid\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
