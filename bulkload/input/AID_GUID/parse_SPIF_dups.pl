...
#SPIF never had records in CCH1

	$otherCatalogNumbers =~ s/^SPIF Accession *\# *//;
	
#no records will be found using the old_AID
	if ($otherCatalogNumbers =~ m/^0*([0-9]+)$/){ #some have leading zeros, delete them
		$oldA = $herb.$1;
		&CCH::log_change("Problem otherCatalogNumbers==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\t$_");
	}
	elsif ($otherCatalogNumbers =~ m/^0*([0-9]+)[A-Za-z-]+$/){ #some have leading zeros, delete them
		$oldA = $herb.$1;
		&CCH::log_change("Problem otherCatalogNumbers==>$CCH2id==>$catalogNumber==>$otherCatalogNumbers\t$_");
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^SPIF(0+[1-9][0-9]*)[A-Za-z-]*$/){
	...
	elsif ($catalogNumber =~ m/^SPIF([1-9][0-9]*)[A-Za-z-]+$/){
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		#print "HERB(3)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
