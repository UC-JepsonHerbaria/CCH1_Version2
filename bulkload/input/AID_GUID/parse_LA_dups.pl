...
#check for nulls
	...
	$otherCatalogNumbers=~ s/^(LAbREEZ|LAB|LA)(\d+)/LA$2/i; #fix some lack of problem accessions with erroneous prefixes

#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){

#Dick loaded the original LA in 2012 and David last loaded an update in 2015
	if ($otherCatalogNumbers =~ m/^LA *0*([1-9][0-9]*)\.[0-9A-Za-z]*$/){
	...
	elsif ($otherCatalogNumbers =~ m/^LA *0*([1-9][0-9]*)$/){
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
		$duplicate_OTH{$old_AID}++;
		#print "HERB(2a)\t$oldA\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n"";
	...
	elsif ($otherCatalogNumbers =~ m/^LA *0*([1-9][0-9]*)-?[A-Za-z]*$/){ 
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
		$oldA =~ s/[A-Za-z]+$//;
		$oldA =~ s/-//;
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^LA(0*[1-9][0-9]*)$/){
