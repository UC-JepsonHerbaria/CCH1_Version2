...
$oldA = "";
	$otherCatalogNumbers=~ s/^MACF$/NULL/i; #fix some problem accessions with only prefixes and no numbers
	$otherCatalogNumbers=~ s/^(M17 *miles *ACF|MACF +|MA[FC]|MACf)(\d+)/MACF$2/; #fix some lack of problem accessions with erroneous prefixes
...
#Leading zeros were left in David's original parsed file, but it is possible that leading zeros were removed occasionally at other stages if loading 
if (length ($otherCatalogNumbers) >= 1){
	...
	if ($otherCatalogNumbers =~ m/NULL/){
		...
		$old_AID=$herb.$1;
		print "HERB(1)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($otherCatalogNumbers =~ m/^(MACF)(\d+)$/){ 
		$old_AID=$1.$2;
