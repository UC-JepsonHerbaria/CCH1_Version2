...
#check for nulls
	...
	$otherCatalogNumbers =~ s/^[A-Za-z]+$//; #fix some problem accessions with only prefixes and no numbers
	$otherCatalogNumbers =~ s/^(M17 *miles *ACF|MACF +|MA[FC]|MACf)(\d+)/MACF$2/i; #fix some lack of problem accessions with erroneous prefixes
	$catalogNumber =~ s/^[A-Za-z]+$//;

#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){
#MACF had records in CCH1, mostly without leading zeros
#MACF was last last loaded in CCH1 in 2015 by David.  CCH1 used the same file as he created and never updated the loading script
#Leading zeros were left in David's original parsed file, 
#but it is possible that leading zeros were removed occasionally at other stages if loading 
	if ($otherCatalogNumbers =~ m/^MACF0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
	...
	elsif ($otherCatalogNumbers =~ m/^0*([1-9][0-9]*)$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
		$duplicate_OTH{$old_AID}++;
		#print "HERB(2)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^MACF(0*[1-9][0-9]*)$/){
