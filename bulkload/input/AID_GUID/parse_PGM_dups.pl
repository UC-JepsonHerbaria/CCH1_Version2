...
open(PGM, ">/Users/Shared/Jepson-Master/CCHV2/bulkload/input/AID_GUID/DUPS/DUPS_".$herb."_all.txt") || die; #this only needs to be active once to generate a list of duplicated accessions
...
#PGM has nothing in othercatalogNumbers
...
#there are hundreds of accessions with letters, so do not strip off
if (length ($catalogNumber) >= 1){

	if ($catalogNumber =~ m/^H-(\d+)-?([AaBbCcDdEeFf])\+?$/){
		$test = $2;
		$l = uc($test);
		$old_AID=$herb.$1.$l;
		$ALT_CCH_BARCODE = "";
		...
		print PGM "$old_AID\t$alt\t$catalogNumber\t$CCH2id\n";
	}
	elsif ($catalogNumber =~ m/^H-(\d+)$/){
		$old_AID=$herb.$1;
		$ALT_CCH_BARCODE = "";
		$alt = $catalogNumber;
		$alt =~ s/[- ]+//g;
		...
		$alt = $catalogNumber;
		#print "HERB(3)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
		print PGM "$old\t$alt\t$catalogNumber\t$CCH2id\n";
	}
	elsif ($catalogNumber =~ m/^H(\d+)([AaBbCcDdEeFf]+)&?B?$/){
		$test = $2;
		$l = uc($test);
		$old=$herb.$1.$l;
		$old_AID = $ALT_CCH_BARCODE = "";
