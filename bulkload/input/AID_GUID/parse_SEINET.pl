...
while(<IN>){
	chomp;
	($IC,$CC,$OIC,$CN,$OCN,$ID,$acc,$alt,$old,@residue)=split(/\t/);
#institutionCode	collectionCode	ownerInstitutionCode	CCH2_catalogNumber	CCH2_catalogNumber	CCH2_ID	OLD_CCH_AID	ALT_CCH_ID	ALT_CCH_AID_SPACE	Status	GUID-occurrenceID

	$DUP{$acc}++; #should not be any dups with this, but who knows?  All SEINET specimens have a value in this field
	if (length ($alt) >= 1){
		$DUPA{$alt}++;
	}
...
#SEINET output is too messy to add to CCH2 data, it has to be treated seperately then merged later
open(OUT3, ">/Users/Shared/Jepson-Master/CCHV2/bulkload/input/AID_GUID/DUPS/DUPS_excluded_$herb.txt") || die;
open(OUT2, ">/Users/Shared/Jepson-Master/CCHV2/bulkload/input/AID_GUID/output/AID_to_ADD_$herb.txt") || die;

	print OUT2 "institutionCode\tcatalogNumber\totherCatalogNumbers\tCCH2_ID\tOLD_CCH_AID\tALT_CCH_ID\tALT_CCH_AID_SPACE\tStatus\tGUID-occurrenceID\tscientificName\tcounty\n";
	print OUT3 "institutionCode\tcatalogNumber\totherCatalogNumbers\tCCH2_ID\tOLD_CCH_AID\tALT_CCH_ID\tALT_CCH_AID_SPACE\tStatus\tGUID-occurrenceID\tscientificName\tcounty\n";
...
$associatedCollectors,	#added 2016, not in 2017 download
$recordNumber,
$eventDate,
#30
...
#id	institutionCode	collectionCode	ownerInstitutionCode	basisOfRecord	occurrenceID	catalogNumber	otherCatalogNumbers	
#kingdom	phylum	class	order	family	scientificName	taxonID	scientificNameAuthorship	genus	specificEpithet	taxonRank	infraspecificEpithet	identifiedBy	dateIdentified	identificationReferences	identificationRemarks	taxonRemarks	identificationQualifier	typeStatus	recordedBy	associatedCollectors	recordNumber	eventDate	year	month	day	startDayOfYear	endDayOfYear	verbatimEventDate	occurrenceRemarks	habitat	substrate	verbatimAttributes	fieldNumber	informationWithheld	dataGeneralizations	dynamicProperties	associatedTaxa	reproductiveCondition	establishmentMeans	cultivationStatus	lifeStage	sex	individualCount	preparations	country	stateProvince	county	municipality	locality	locationRemarks	localitySecurity	localitySecurityReason	decimalLatitude	decimalLongitude	geodeticDatum	coordinateUncertaintyInMeters	verbatimCoordinates	georeferencedBy	georeferenceProtocol	georeferenceSources	georeferenceVerificationStatus	georeferenceRemarks	minimumElevationInMeters	maximumElevationInMeters	minimumDepthInMeters	maximumDepthInMeters	verbatimDepth	verbatimElevation	disposition	language	recordEnteredBy	modified	sourcePrimaryKey-dbpk	collId	recordId	references
#149338	NMC			PreservedSpecimen		39710		
#Plantae	Magnoliophyta		Fabales	Fabaceae	Astragalus trichopodus var. phoxus	74131	(M.E. Jones) Barneby	Astragalus	trichopodus	var.	phoxus	S.L. Welsh	no date						E.K. Balls and L.W. Lenz		14460	1950-03-28	1950	3	28	87				Valley grassland; dry shaley banks, sunny.		Flowers pale yellow; dense bushy tufts.					With Lupinus succulentus.								United States	California	Los Angeles		3 mi N of Castaic, Hwy. 99.														457					1500ft				2015-05-02 02:30:11	39710	22	urn:uuid:89af6182-aad1-4616-bf3b-7bf682561d97	http://swbiodiversity.org/seinet/collections/individual/index.php?occid=149338
#155912	ASU	CCH		PreservedSpecimen	92172013-2715-4e7d-8cf1-f4dd090decf5		71	
#Plantae	Magnoliophyta		Caryophyllales	Chenopodiaceae	Atriplex semibaccata	138	R. Br.	Atriplex	semibaccata			
#N. Cooper							Norman C. Cooper		889	1942-11-09	1942	11	9	313				Salt marshes along the coast.															USA	California	ORANGE		Balboa.				33.6	-118														 ft				2007-07-27 00:00:00	636	15	urn:uuid:92172013-2715-4e7d-8cf1-f4dd090decf5	http://swbiodiversity.org/seinet/collections/individual/index.php?occid=155912
...
#Remove duplicates
	if($DUP{$old_AID}){
		print OUT3 "$institutionCode\t$catalogNumber\t$otherCatalogNumbers\tNULL\t$old_AID\t$ALT_CCH_BARCODE\t$oldA\tDUP\t$occurrenceID\t$scientificName\t$county\n";
#print "EXCL $old_AID\n";
++$dups;
	}
	else{
		print OUT2 "$institutionCode\t$catalogNumber\t$otherCatalogNumbers\tNULL\t$old_AID\t$ALT_CCH_BARCODE\t$oldA\tNONDUP\t$occurrenceID\t$scientificName\t$county\n";
#print "INCL $old_AID\n";
++$included;
	}
}
