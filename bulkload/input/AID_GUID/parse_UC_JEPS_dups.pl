...
#THERE ARE NO DUPS IN CCH2 FOR UCJEPS, 
#therefore the dup list HAS to come from somewhere else.
#it is possible that one of each of the dups has been mistskenly added to CCH2
#import the dups file from the upload process to extract any of the known duplicates from
#CCH2 output


#counts
my ($skipped, $included, %seen, %duplicate_FOUND_CAT, %duplicate_FOUND_OTH,$dupid) = "";
...
open (IN, '/Users/Shared/Jepson-Master/CCH/SYMBIOTA/LOADERS/UCJEPS/output/UCJEPS_DUP_LIST_UNIQUE.txt') or die $!;
Record: while(<IN>){
	chomp;
...
#warn "$count_record\n" unless $count_record % 1009;
  printf {*STDERR} "%d\r", ++$count_record;
	...
	if ($dupid =~ m/^(UC|JEPS)\d+/){
		++$dups;
		#$dupid =~ s/(\t|\n|\r)+//g;
		$duplicate_CAT{$dupid}++;#count to find duplicates
		#print "HERB(1)\t$dupid\n";
	}
	elsif ($dupid =~ m/^UCLA\d+/){
		++$dups;
		$duplicate_CAT{$dupid}++;#count to find duplicates
		$duplicate_OTH{$dupid}++;#some UCLA have been moved to othernumber
		#print "HERB(2)\t$dupid\n";
	}
	else{
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(UC|JEPS)([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $1.$2;
		$ALT_CCH_BARCODE = $1.$2."-BARCODE";
	}
	elsif ($catalogNumber =~ m/^(UCLA)(\d+)$/){
		$old_AID = "UCLA".$2;
		$ALT_CCH_BARCODE = "";
	}
...
This count is different thqn the total above for UCJEP
The DUP list is derived from all of CSpace, 
 which will always be greater than the count found
The ones excluded are only the ones that passed into CCH2
