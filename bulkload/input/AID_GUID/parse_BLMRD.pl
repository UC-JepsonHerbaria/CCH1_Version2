...
#perl -0777 -i.bck  -p -e "s/AID_to_ADD_\'\.\$dupfiledate/AID_to_ADD_CCH2_mod\'\.\$dupfiledate/g;" *.pl
...
#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){
	...
	elsif ($otherCatalogNumbers =~ m/^(CA)([0-9-]+)$/){
		$old_AID="";
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(BLMRD)(0*[1-9][0-9]*)[A-Za-z]*$/){
