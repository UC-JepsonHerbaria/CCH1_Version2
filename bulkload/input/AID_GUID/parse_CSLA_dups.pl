...
#extract old herbarium and aid numbers
   ...
   #CSLA was not in CCH, so there is no old resources to map to their records
   #delete extraneous text
	$otherCatalogNumbers =~ s/^Californi[as] State (College|University|Colleg0e) Lo[as] Angeles: //i;
	$otherCatalogNumbers =~ s/^Californi[as] State (College|University|Colleg0e) at Lo[as] Angeles: //i;
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^(CSLA)(0*[1-9][0-9]*)$/){
		$old_AID = $1.$2;
		$ALT_CCH_BARCODE = $old_AID."-BARCODE";
