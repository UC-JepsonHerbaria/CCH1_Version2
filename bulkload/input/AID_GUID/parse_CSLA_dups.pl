...
#CSLA appears not to have anything in othercatalogNumbers, they also never had records in CCH1
...
#find duplicates
if ($old_AID !~ m/^ *$/){
	if($duplicate{$old_AID}++){#CSLA does not have any othercatalognumbers, so test for dups on the catalognumber
	++$dups;
