...
#CCH2 ccid 3991113-3992155 are unexplained skeletal copies of accessions already loaded, 
#not true duplicates, so they need to be skipped.
	my $a = 3991113;
	my $b = 3992155;
	#this is experimental
	#if($CCH2id ~~ [$a..$b]){
	#	&CCH::log_skip("Skeletal Record with an erroseously duplicate CCH2 occid and no GUID $_");
	#	++$skipped;
	#	next Record;
	#}
	
	if (($CCH2id >= m/^$a$/) && ($CCH2id <= m/^$b$/)){
		$oldA = $old_AID = "";
		$ALT_CCH_BARCODE = $herb.$catalogNumber."-BARCODE";
...
#construct the 3rd variant, which is a form without leading zeros
	if ($otherCatalogNumbers =~ m/^SD *0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
		$oldA =~ s/[A-Za-z]+$//;
	}
	elsif ($otherCatalogNumbers =~ m/^SD *([1-9][0-9]*)[A-Za-z]*$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^SD(0*[1-9][0-9]*)$/){
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
	}
	elsif ($catalogNumber =~ m/^\d+SD(0*[1-9][0-9]*)$/){
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
	}
