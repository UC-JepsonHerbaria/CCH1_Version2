...
my $oldA="";
	$catalogNumber =~ s/^(SCFS5071 *[^ ] *SCFS5072)$/SCFS5071/;
	$catalogNumber =~ s/^SCF(\d+)/SCFS$1/;

#extract old herbarium and aid numbers
if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^(SCFS)([0-9]+)$/){ 
		$old_AID=$1.$2;
...
if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^(SCFS)(\d+)[AaBbRr]?$/){
		$ALT_CCH_BARCODE=$1.$2;
