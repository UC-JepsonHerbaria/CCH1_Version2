...
#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){
	...
	if ($otherCatalogNumbers =~ m/^SCFS0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
	...
	elsif ($otherCatalogNumbers =~ m/^SCFS([1-9][0-9]*)[A-Za-z]*$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
		$duplicate_OTH{$old_AID}++;
	...
	elsif ($otherCatalogNumbers =~ m/^([0-9]+)$/i){ 
		$old_AID = $oldA = "";
		#print "HERB(2)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($otherCatalogNumbers =~ m/^[hp]+([1-9][0-9]*)$/i){ 
		$old_AID = $oldA = "";
		#print "HERB(2)\t$old_AID\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^SCFS(0+)([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$2;
		$ALT_CCH_BARCODE = $herb.$1.$2;
	...
	elsif ($catalogNumber =~ m/^SCFS([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = "";
