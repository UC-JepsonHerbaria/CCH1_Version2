...
($EJD, $LJD)=&CCH::make_date_ids($DD2, $MM2, $YYYY2, $DD, $MM, $YYYY);

my $today_JD = &CCH::get_today;
my $today_EP = &CCH::get_today_epoch;
...
($EJD, $LJD,$dateFlag)=&CCH::check_dates($EJD,$LJD);
print "LJD-->".$dateFlag." ".$LJD."\n";
$today_JD =~ s/ *PDT//;
...
#VVC appears not to have anything in othercatalogNumbers
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^VVC-?(0+)([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$2;
		$oldA = $catalogNumber;
		$ALT_CCH_BARCODE="";
	}
	elsif ($catalogNumber =~ m/^VVC-?([1-9][0-9]*)[A-Za-z-]*$/){
		$old_AID = $herb.$1;
		$oldA = "";
		$ALT_CCH_BARCODE="";
	}
