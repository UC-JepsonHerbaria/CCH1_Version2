...
#perl -0777 -p -i -e 's/my \(\$rightsHolder,\$rights,\$associatedOccurrences,\$eventID,\$associatedSequences\) = "";\n\n/my (\$rightsHolder,\$rights,\$associatedOccurrences,\$eventID,\$associatedSequences) = "";\nmy (\$locationID,\$continent,\$waterBody,\$islandGroup,\$island,\$eventDate2) = "";/g;' *dups.pl
...
#BFRS does not have other catalog numbers filled out currently, old_aid=CatalogNumber
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^BFRS0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
	...
	elsif ($catalogNumber =~ m/^BFRS([1-9][0-9]*)[A-Za-z]*$/){
		$ALT_CCH_BARCODE = $herb.$1."-BARCODE";
		$old_AID = $herb.$1;
		$duplicate_CAT{$ALT_CCH_BARCODE}++;#count to find duplicates
		$duplicate_OTH{$old_AID}++;#count to find duplicates
...
#exclude ALL duplicates 
	...
	#for example, the worst case is CAS520791, which has 17 specimens that were labeled with this number over the years.
		print OUT "$institutionCode\t$catalogNumber\t$otherCatalogNumbers\t$id\t$old_AID\t$ALT_CCH_BARCODE\t$oldA\tDUP\t$occurrenceID\t$scientificName\t$verbatimCounty\n";
		++$dups_B;
	}
