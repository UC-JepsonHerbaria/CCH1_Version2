...
#LOB has only numbers in othercatalogNumbers, they also never had records in CCH1
#no records will be found using the old_AID
if (length ($otherCatalogNumbers) >= 1){
	if ($otherCatalogNumbers =~ m/^(NULL|NON[eE]|`)$/){
		$otherCatalogNumbers = "";
		$old_AID="";
	}
	elsif ($otherCatalogNumbers =~ m/^\d+, +\d*[, ]*\d* ?[NONE]*$/){
		#print "HERB(1a)\tNULL\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		$otherCatalogNumbers = "";
		$old_AID="";
	}
	elsif ($otherCatalogNumbers =~ m/^\d+[ ]+\d+$/){
		#print "HERB(1b)\tNULL\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		$otherCatalogNumbers = "";
		$old_AID="";
	}
	elsif ($otherCatalogNumbers =~ m/^(\d+),(\d+)$/){
		print "HERB(1c)\tNULL\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
		$old_AID=$herb.$1.$2;
	}
	elsif ($otherCatalogNumbers =~ m/^[LOB]+(\d+)[AaBbCc]?$/){ 
		$old_AID=$herb.$1;
