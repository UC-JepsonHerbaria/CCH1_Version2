...
#construct the 3rd variant, which is a form without leading zeros
	if ($otherCatalogNumbers =~ m/^SDSU0+([1-9][0-9]*)[A-Za-z]*$/){
		$old_AID = $herb.$1;
		$oldA = $otherCatalogNumbers;
		$oldA =~ s/[A-Za-z]+$//;
	}
	elsif ($otherCatalogNumbers =~ m/^SDSU([1-9][0-9]*)[A-Za-z]*$/){ 
		$old_AID = $herb.$1;
		$oldA = "";
	}
	elsif ($otherCatalogNumbers =~ m/^(SDSU)([0-9]+)[, ]+\d+$/){ 
		$old_AID = $1.$2;
		$oldA = $otherCatalogNumbers;
		$oldA =~ s/,//g;
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	...
	if ($catalogNumber =~ m/^SDSU(0*[1-9][0-9]*)$/){
