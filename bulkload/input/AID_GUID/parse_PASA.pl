...
my $oldA="";
	$otherCatalogNumbers =~ s/^(y\.?\d+|495-54|195-54)$/NULL/;#delete this single repetitive accession that is showing across multiple records; looks like an error
	$otherCatalogNumbers =~ s/^P\d+$/NULL/;#delete this repetitive accession that is showing across multiple records; looks like an error
	$otherCatalogNumbers =~ s/^(Peirson Herbarium No\.10576|frag pkt 3011|5170 of.+|1845 of.+)$/NULL/; #delete problem accessions

#extract old herbarium and aid numbers
#PASA has some numbers in otherCatalogNumbers, not sure what they are, most of them are deleted herein
#no records will be found using the old_AID here, moved to catalogNumbers
if (length ($otherCatalogNumbers) >= 1){

	if ($otherCatalogNumbers =~ m/^(\d+)$/){
		$old_AID=$herb.$1;
	}
