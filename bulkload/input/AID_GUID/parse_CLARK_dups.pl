...
#extract old herbarium and aid numbers
   if (length ($otherCatalogNumbers) >= 1){
	if ($otherCatalogNumbers =~ m/^(A0*[1-9][0-9-]*)[A-Za-z]*$/){#unique to CLARK
		$oldA = $herb."-".$otherCatalogNumbers;
		$old_AID = $herb."-".$1;
		#$duplicate_CAT{$old_AID}++;
		#print "HERB(2A)\t$oldA\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
	}
	elsif ($otherCatalogNumbers =~ m/^0*([1-9][0-9-]*)[A-Za-z]*$/){ #unique to CLARK
		$oldA = $herb."-".$otherCatalogNumbers;
		$old_AID = $herb."-".$1;
		$duplicate_CAT{$old_AID}++;
		print "HERB(2B)\t$oldA\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n";
	}
...
#construct catalog numbers
   if (length ($catalogNumber) >= 1){
	if ($catalogNumber =~ m/^CLARK-(A0*[1-9][0-9-]*)[A-Za-z]*$/){
		$ALT_CCH_BARCODE = ""; #no barcodes in CLARK
		$old_AID = $herb."-".$1;
		$oldA = "";
		$duplicate_CAT{$old_AID}++;
		#print "HERB(3)\t$ALT_CCH_BARCODE\t$CCH2id==>$catalogNumber==>$otherCatalogNumbers\n" if $otherCatalogNumbers =~ m/98/g;
	}
	elsif ($catalogNumber =~ m/^(A?0*[1-9][0-9]*)$/){#fix records without an herbarium code
		$ALT_CCH_BARCODE = ""; #no barcodes in CLARK
		$old_AID = $herb."-".$1;
		$oldA = "";
